...
async def test_micron_bolt_mini_2_no_connection(mocker: MockerFixture):
    ...
    # no connection
    assert protocol.datagram_received(b"+SACK:GTHBD,423136,010C$\r\n", addr) is None
...
async def test_micron_bolt_mini_2_gps(mocker: MockerFixture):
    ...
    protocol.connection_made(transport)
    protocol.datagram_received(
        b"+RESP:GTFRI,423136,352009117419957,,0,0,1,1.0,0.0,62,28.3,-122.990623,38.313342,20230414193420,310,260,38F2,2D01A05,00,77,20230414193424,0297$",
        addr,
    )
...
async def test_micron_bolt_mini_2_wifi_no_api_key(mocker: MockerFixture):
    ...
    protocol.connection_made(transport)
    protocol.datagram_received(
        b"+RESP:GTWIF,423136,352009117419957,,4,b0e4d556fbc6,-58,,,,a4d79504102d,-73,,,,322f23291e0b,-78,,,,44a56edcdae8,-88,,,,,,,,77,20230414193408,0295$",
        addr,
    )
    assert queue.empty()
